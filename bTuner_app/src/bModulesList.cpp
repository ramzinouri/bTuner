...
void bModulesList::DrawItem(WPARAM wParam, LPARAM lParam)
{
	...
	TextOut(pdis->hDC, 5, pdis->rcItem.top+10, ms, ms.GetLength());

}

void bModulesList::OnCreate()
{
	MoveWindow(0, 35, 150, GetParent().GetClientRect().bottom - 185);
}

void bModulesList::PreCreate(CREATESTRUCT & cs)
{
	cs.style = WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_BORDER | LBS_NOTIFY | LBS_HASSTRINGS | LBS_OWNERDRAWFIXED;
}
